...
App({
  onLaunch: function () {
    util.mac2DeviceId('2B021DD25B4C');
    if (!wx.cloud) {
    ...
    wx.onBluetoothDeviceFound((result) => {
      ...
      for (var i = 0; i < result.devices.length; i++) {
        ...
        if ((mfrHead == 'SB1' || mfrHead == 'SB2' || mac == '2B021DD25B4C') && TYPE_PATTERN.test(deviceType)) {
  ...
  globalData: {
    queryValue: null,//查询状态的指令，为了减少加密API的使用量，就保持一个加密后的指令来定时获取
    openid: '',
    myuser: {},
    isNetworkOn: true,
    appHidden: true,
