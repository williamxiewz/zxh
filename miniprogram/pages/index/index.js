...
Page({
  ...
  onShow: function () {
    ...
    // console.error('xxx isUserAvailable=' + app.isUserAvailable());

    let isCall = !selectedDevice || selectedDevice.name.indexOf('XY2') == -1;
    that.setData({
      logo: sputil.getLogo(),
      connected: isConnected,
      imageOfRbBtn: this.getCallImage(isCall, this.data.imageOfRbBtn),
      isCall: isCall
    })
  ...
  onTouchEnd: function (e) {
    ...
    let index = parseInt(e.currentTarget.dataset.btnindex)
    if(this.data.isStart && index == 0) {
      index = 1;//启动状态下重复点击启动按钮执行解锁功能
    }
  ...
  setImage: function (index, imageurl) {
    ...
    switch (index) {
      ...
      case 4: //RB
        imageurl = this.getCallImage(this.data.isCall, imageurl);
        this.setData({
  ...
  getCallImage(isCall, imageurl) {
    if (!isCall && imageurl.indexOf('XY2') == -1) {
